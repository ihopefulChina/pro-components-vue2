...
export interface SearchFormConfig {
  ...
  // 选择器选项
  valueEnum?:
    Array<{ label: string; value: any }> | Record<string, string | { label: string; value: any }>;
  // 搜索表单项验证规则
  searchRules?: Array<{
    required?: boolean;
    message?: string;
...
export interface ICommonTableColumn {
  ...
  searchLabel?: string;
  // 搜索表单项类型
  valueType?: "input" | "textarea" | "select" | "date" | "daterange" | "number" | "image" | "link";
  // 选择器选项（当 valueType 为 select 时）
  valueEnum?:
    | Array<{ label: string; value: any }>
    | Record<
        string | number,
        string | { label: string; value?: any; status?: "success" | "warning" | "danger" | "info" }
      >;
