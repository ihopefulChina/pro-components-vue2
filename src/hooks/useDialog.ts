...
import type { ElMessageBoxShortcutMethod } from "element-ui/types/message-box";
...
export function useDialog(): DialogInstance {
  ...
  const confirmApi = (proxy as typeof proxy & { $confirm: ElMessageBoxShortcutMethod }).$confirm;

  const confirm = async (config: DialogConfig): Promise<boolean> => {
    try {
      await confirmApi(config.content || "", config.title || "提示", {
